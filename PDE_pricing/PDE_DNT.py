...
def gen_bound(M, n):
    ...
    f_mat[0,:] = R * T * np.exp(-r * (T-np.arange(M+1)/M * T)) * s0
    # f(t_i, S_max) = R * T * exp{-r(T-t)}
    f_mat[-1, n * 30 * np.arange(3,13)] = R * T * np.exp(-r * (T - np.arange(3,13) * 30/ 365)) * s0
...
def calculate_f_matrix(M):
    ...
    for i in range(M, 0, -1):
        ...
        if (i-1)%(30*n)==0 and i-1 >= 30 * 3 * n:
            Fi_1[bar_out:] = R * T * np.exp(-r * (1-(i-1)/M) * T) * s0
        if i-1 >= 30 * 3 * n:
            Fi_1[:bar_in] = R * T * np.exp(-r * (1-(i-1)/M) * T) * s0
        Fi_1 = list(np.array(Fi_1.reshape(1, M - 1))[0])
        f_matrix[1:M, i-1]=Fi_1
    # 这一步取出S_t在网格中的位置，然后抽出结果，即为在该股价的期权价格。
    i = np.round((s0 - s_min) / delta_S, 0)
    return f_matrix[int(i), 0]

DOT = calculate_f_matrix(M)
DNT = R * T * np.exp(-r * T) - DOT